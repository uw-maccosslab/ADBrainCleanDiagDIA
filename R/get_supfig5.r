...
get_supfig5 <- function() {
  ...
  filter = (apply(protGrp, 1, function(x) sum(is.na(x)))) > (ncol(protGrp) - 9)
  noBatchProt[!filter,]
  protGrp[!filter,]
  ...
  ## Pre-Batch Adjust PVCA #####################################################
  {
    ...
    pct_threshold = .5876 # Amount of variability desired to be explained by the principal components.
    
    theDataMatrix <- protGrp
